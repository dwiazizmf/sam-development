    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.invoices.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('polis') ? 'is-invalid' : '' }}" name="polis_id" id="polis_id" required>
                    @foreach($polis as $id => $entry)
                        <option value="{{ $id }}" {{ old('polis_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('polis'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subtotal_amount') ? 'is-invalid' : '' }}" type="number" name="subtotal_amount" id="subtotal_amount" value="{{ old('subtotal_amount', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('paid_at') ? 'is-invalid' : '' }}" type="text" name="paid_at" id="paid_at" value="{{ old('paid_at') }}">
