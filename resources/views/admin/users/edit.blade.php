...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="commission_rate">{{ trans('cruds.user.fields.commission_rate') }}</label>
                <input class="form-control {{ $errors->has('commission_rate') ? 'is-invalid' : '' }}" type="number" name="commission_rate" id="commission_rate" value="{{ old('commission_rate', $user->commission_rate) }}" step="0.01">
                @if($errors->has('commission_rate'))
                    <span class="text-danger">{{ $errors->first('commission_rate') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.commission_rate_helper') }}</span>
            </div>
            <div class="form-group">
