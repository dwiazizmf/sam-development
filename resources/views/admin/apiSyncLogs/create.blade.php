...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.apiSyncLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.api-sync-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="system_name">{{ trans('cruds.apiSyncLog.fields.system_name') }}</label>
                <input class="form-control {{ $errors->has('system_name') ? 'is-invalid' : '' }}" type="text" name="system_name" id="system_name" value="{{ old('system_name', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('endpoint') ? 'is-invalid' : '' }}" type="text" name="endpoint" id="endpoint" value="{{ old('endpoint', '') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('request_data') ? 'is-invalid' : '' }}" name="request_data" id="request_data">{{ old('request_data') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('response_code') ? 'is-invalid' : '' }}" type="text" name="response_code" id="response_code" value="{{ old('response_code', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ApiSyncLog::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'success') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('error_message') ? 'is-invalid' : '' }}" name="error_message" id="error_message">{{ old('error_message') }}</textarea>
