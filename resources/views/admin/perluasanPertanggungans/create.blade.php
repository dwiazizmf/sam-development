...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.perluasanPertanggungan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.perluasan-pertanggungans.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="pertanggungan_name">{{ trans('cruds.perluasanPertanggungan.fields.pertanggungan_name') }}</label>
                <input class="form-control {{ $errors->has('pertanggungan_name') ? 'is-invalid' : '' }}" type="text" name="pertanggungan_name" id="pertanggungan_name" value="{{ old('pertanggungan_name', '') }}">
