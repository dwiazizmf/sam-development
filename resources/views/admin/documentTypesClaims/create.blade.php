...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.documentTypesClaim.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.document-types-claims.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('document_name') ? 'is-invalid' : '' }}" type="text" name="document_name" id="document_name" value="{{ old('document_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('claim_type_group') ? 'is-invalid' : '' }}" name="claim_type_group_id" id="claim_type_group_id">
                    @foreach($claim_type_groups as $id => $entry)
                        <option value="{{ $id }}" {{ old('claim_type_group_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('claim_type') ? 'is-invalid' : '' }}" name="claim_type_id" id="claim_type_id">
                    @foreach($claim_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('claim_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('file_format_allowed') ? 'is-invalid' : '' }}" type="text" name="file_format_allowed" id="file_format_allowed" value="{{ old('file_format_allowed', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max_file_size_mb') ? 'is-invalid' : '' }}" type="number" name="max_file_size_mb" id="max_file_size_mb" value="{{ old('max_file_size_mb', '1') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_image_only') ? 'is-invalid' : '' }}" name="is_image_only" id="is_image_only" required>
                    ...
                    @foreach(App\Models\DocumentTypesClaim::IS_IMAGE_ONLY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_image_only', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('require_original') ? 'is-invalid' : '' }}" name="require_original" id="require_original" required>
                    ...
                    @foreach(App\Models\DocumentTypesClaim::REQUIRE_ORIGINAL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('require_original', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('validation_rules') ? 'is-invalid' : '' }}" name="validation_rules" id="validation_rules">{{ old('validation_rules') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sample_document_path') ? 'is-invalid' : '' }}" type="text" name="sample_document_path" id="sample_document_path" value="{{ old('sample_document_path', '') }}">
