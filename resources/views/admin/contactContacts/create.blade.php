    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contact-contacts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_phone_1') ? 'is-invalid' : '' }}" type="text" name="contact_phone_1" id="contact_phone_1" value="{{ old('contact_phone_1', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('lead_source') ? 'is-invalid' : '' }}" name="lead_source" id="lead_source">
                    ...
                    @foreach(App\Models\ContactContact::LEAD_SOURCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('lead_source', 'referral') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lead_source_detail') ? 'is-invalid' : '' }}" type="text" name="lead_source_detail" id="lead_source_detail" value="{{ old('lead_source_detail', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('potential_revenue') ? 'is-invalid' : '' }}" type="number" name="potential_revenue" id="potential_revenue" value="{{ old('potential_revenue', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estimated_policies_per_month') ? 'is-invalid' : '' }}" type="text" name="estimated_policies_per_month" id="estimated_policies_per_month" value="{{ old('estimated_policies_per_month', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" name="priority" id="priority">
                    ...
                    @foreach(App\Models\ContactContact::PRIORITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('priority', 'low') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status_prospect') ? 'is-invalid' : '' }}" name="status_prospect_id" id="status_prospect_id">
                    @foreach($status_prospects as $id => $entry)
                        <option value="{{ $id }}" {{ old('status_prospect_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
