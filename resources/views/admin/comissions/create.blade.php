...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.comission.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.comissions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assigned_to_users') ? 'is-invalid' : '' }}" name="assigned_to_users[]" id="assigned_to_users" multiple>
                    @foreach($assigned_to_users as $id => $assigned_to_user)
                        <option value="{{ $id }}" {{ in_array($id, old('assigned_to_users', [])) ? 'selected' : '' }}>{{ $assigned_to_user }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from_users') ? 'is-invalid' : '' }}" name="from_users[]" id="from_users" multiple>
                    @foreach($from_users as $id => $from_user)
                        <option value="{{ $id }}" {{ in_array($id, old('from_users', [])) ? 'selected' : '' }}>{{ $from_user }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('polis_transactions') ? 'is-invalid' : '' }}" name="polis_transactions[]" id="polis_transactions" multiple>
                    @foreach($polis_transactions as $id => $polis_transaction)
                        <option value="{{ $id }}" {{ in_array($id, old('polis_transactions', [])) ? 'selected' : '' }}>{{ $polis_transaction }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('level') ? 'is-invalid' : '' }}" type="number" name="level" id="level" value="{{ old('level', '1') }}" step="1">
                @if($errors->has('level'))
