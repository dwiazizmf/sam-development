    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contact-companies.update", [$contactCompany->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('business_type') ? 'is-invalid' : '' }}" name="business_type_id" id="business_type_id" required>
                    @foreach($business_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('business_type_id') ? old('business_type_id') : $contactCompany->business_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_telp') ? 'is-invalid' : '' }}" type="text" name="no_telp" id="no_telp" value="{{ old('no_telp', $contactCompany->no_telp) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text" name="city" id="city" value="{{ old('city', $contactCompany->city) }}">
                @if($errors->has('city'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('province') ? 'is-invalid' : '' }}" type="text" name="province" id="province" value="{{ old('province', $contactCompany->province) }}">
