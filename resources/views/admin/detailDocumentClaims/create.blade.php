...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.detailDocumentClaim.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.detail-document-claims.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('policies_data') ? 'is-invalid' : '' }}" name="policies_data_id" id="policies_data_id">
                    @foreach($policies_datas as $id => $entry)
                        <option value="{{ $id }}" {{ old('policies_data_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('claims') ? 'is-invalid' : '' }}" name="claims_id" id="claims_id">
                    @foreach($claims as $id => $entry)
                        <option value="{{ $id }}" {{ old('claims_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
