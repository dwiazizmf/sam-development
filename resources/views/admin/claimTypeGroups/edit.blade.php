...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.claimTypeGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.claim-type-groups.update", [$claimTypeGroup->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="claim_group_code">{{ trans('cruds.claimTypeGroup.fields.claim_group_code') }}</label>
                <input class="form-control {{ $errors->has('claim_group_code') ? 'is-invalid' : '' }}" type="text" name="claim_group_code" id="claim_group_code" value="{{ old('claim_group_code', $claimTypeGroup->claim_group_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('claim_group_name') ? 'is-invalid' : '' }}" type="text" name="claim_group_name" id="claim_group_name" value="{{ old('claim_group_name', $claimTypeGroup->claim_group_name) }}" required>
