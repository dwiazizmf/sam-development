    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.claims.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('policies') ? 'is-invalid' : '' }}" name="policies_id" id="policies_id">
                    @foreach($policies as $id => $entry)
                        <option value="{{ $id }}" {{ old('policies_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('claim_status') ? 'is-invalid' : '' }}" name="claim_status" id="claim_status" required>
                    ...
                    @foreach(App\Models\Claim::CLAIM_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('claim_status', 'draft') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reviewed_by_user') ? 'is-invalid' : '' }}" name="reviewed_by_user_id" id="reviewed_by_user_id">
                    @foreach($reviewed_by_users as $id => $entry)
                        <option value="{{ $id }}" {{ old('reviewed_by_user_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('review_date') ? 'is-invalid' : '' }}" type="text" name="review_date" id="review_date" value="{{ old('review_date') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('review_notes') ? 'is-invalid' : '' }}" name="review_notes" id="review_notes">{{ old('review_notes') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('approved_amount') ? 'is-invalid' : '' }}" type="number" name="approved_amount" id="approved_amount" value="{{ old('approved_amount', '') }}" step="0.01">
